...
data class FiltersDto(
    @Json(name = "countries") val countries: List<FilterCountryDto>,
    @Json(name = "genres") val genres: List<FilterGenreDto>
)
object FilmFilters {
    @Suppress("SpellCheckingInspection")
    private const val filtersJson = """{
        "genres": [
        {
            "id": 1,
            "genre": "триллер"
    ...
    private val filtersAdapter:JsonAdapter<FiltersDto> = moshi.adapter(FiltersDto::class.java)
    private val filters =  filtersAdapter.fromJson(filtersJson)
    private var genres = filters!!.genres.toMutableList()
    fun getRandomGenre():FilterGenreDto {
        if (genres.isEmpty()) genres = filters!!.genres.toMutableList()
